...
case_name = 'HALE_with5control_surfaces'
...
def generate_aero_file():
    ...
    control_surface_deflection[0] = rudder_static_deflection
    control_surface_chord[0] = m
    ...
    we = 0
    wn = 0
    # right wing (surface 0, beam 0) with 2 control surface positioned the most right
    i_surf = 0
    ...
    for i_elem in range(we+4, we + 6):
        for i_local_node in range(n_node_elem):
            control_surface[i_elem, i_local_node] = 0
    for i_elem in range(we+6, we + 8):
        for i_local_node in range(n_node_elem):
    ...
    for i_elem in range(we, we + n_elem_fin):
        for i_local_node in range(n_node_elem):
            ...
            control_surface[i_elem, i_local_node] = 2
    ...
    for i_elem in range(we, we + n_elem_tail):
        for i_local_node in range(n_node_elem):
            ...
            control_surface[i_elem, i_local_node] = 3
...
def generate_solver_file():
    ...
    settings['DynamicCoupled'] = {'print_info': 'on',
                                  ...
                                  'postprocessors': ['WriteVariablesTime','BeamLoads','BeamPlot', 'AerogridPlot'],
    ...
    settings['WriteVariablesTime'] = {'structure_variables': ['pos'],
                                        'structure_nodes': list(range(0, n_node+1)),
                                        'cleanup_old_solution': 'on',
                                        }
